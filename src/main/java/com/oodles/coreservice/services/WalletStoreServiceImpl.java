 ...
 */
@Service
public class WalletStoreServiceImpl implements  WalletStoreService{
	private static Logger log = LoggerFactory.getLogger(WalletStoreServiceImpl.class);
	@Autowired
	WalletDao walletDao;
	@Autowired
	...
	/**
	 * this map will keep the listener of the wallet
	 */
	private static Map<String, CoinReceiveListner> walletAndListenerMap = new HashMap();
	...
	public void walletListner(Wallet wallet, WalletInfo walletInfo) {
		...
		CoinReceiveListner obj = new CoinReceiveListner(walletInfo, wallet);
		walletAndListenerMap.put(walletInfo.getWalletUuid(), obj);
		wallet.addCoinsReceivedEventListener(obj);
